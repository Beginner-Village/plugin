...
from hiagent_plugin_webscraper.third_party.dify.extractor import extract_processor
from hiagent_plugin_webscraper.third_party.dify.extractor.extract_processor import ExtractProcessor
...
def get_url(url: str, user_agent: str = None) -> str:
    ...
    if main_content_type in extract_processor.SUPPORT_URL_CONTENT_TYPES:
        # fix https://arxiv.org/pdf/1706.03762 not has suffix ".pdf"
        if main_content_type == "application/pdf":
            return ExtractProcessor.load_from_url(url, return_text=True, override_suffix=".pdf")
        else:
...
def find_module_path(module_name):
    # fix: use sys path not site.getsitepackages(). package may installed in target dir
    import sys
